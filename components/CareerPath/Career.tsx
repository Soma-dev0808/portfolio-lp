...
const Career: React.FC<CareerProps> = ({
  ...
  const stepsContainerOnFocus = isFocus
    ? careerPathStyle.stepsContainerFocus
    : "";
  const contentOnFocus = isFocus ? careerPathStyle.contentFocus : "";

  return (
    <div
      className={`${careerPathStyle.stepsContainer} ${stepsContainerOnFocus}`}
    >
      <div className={`${careerPathStyle.content} ${contentOnFocus}`}>
        <h3>{position}</h3>
        <p className={careerPathStyle.companyInfo}>
          <span className={careerPathStyle.companyName}>＠{companyName}</span>
          {" -"} {companyLocation}
        </p>
        <ul
          ref={ref}
          className={careerPathStyle.workDetailList}
          style={{
            maxHeight: isFocus ? ref.current?.scrollHeight : "150px",
          }}
        >
          {workDetails.map((wd) => (
